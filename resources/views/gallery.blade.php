...
@section("content")
    ...
    <div class="dropdown dropright">
        <button id="art-list-btn" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-expanded="false"
                type="button">{{ $artist->name }}</button>
        <div class="dropdown-menu" role="menu" id="art-list">
            @foreach ($artists as $art)
                <a class="dropdown-item" role="presentation" id="{{ $art->id }}"
                   onclick="galleryAjax(this)">{{ $art->name }}</a> @endforeach
        </div>
    </div>
    <div class="row">
